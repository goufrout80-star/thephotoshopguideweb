...
const layers = [
  { icon: Image, label: 'Background', tone: 'bg-panel-2', offset: 0 },
  { icon: Wand2, label: 'Retouch — Skin', tone: 'bg-panel-2', offset: 1 },
  { icon: Palette, label: 'Color Grade', tone: 'bg-coral-soft', offset: 2 },
  { icon: Sliders, label: 'Curves + Dodge', tone: 'bg-cyan-soft', offset: 3 },
]

const toolCopy = {
  move: 'Move — drag the canvas to look around',
  lasso: 'Lasso — selecting the subject',
  wand: 'Magic Wand — selecting by color',
  brush: 'Brush — retouching skin tones',
  eraser: 'Eraser — revealing the layer below',
  eyedropper: 'Eyedropper — sampling a swatch',
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          data-brush-cursor
          className="absolute inset-6 rounded-xl overflow-hidden bg-gradient-to-b from-panel-2 to-canvas-soft cursor-none"
        >
          ...
          <AnimatePresence>
            {activeTool === 'wand' && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-x-0 bottom-0 h-3/4 bg-cyan/15"
              />
            )}
          ...
          {activeTool === 'eyedropper' && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className="absolute left-3 bottom-14 h-6 w-6 rounded-full border-2 border-ink"
            >
              <span className="absolute inset-1 rounded-full bg-cyan" />
            </motion.div>
          )}
        </div>
        ...
        {/* Swatches cluster — a nod to the Photoshop color picker */}
        <motion.div
        ...
        >
          {['#34d3ff', '#ff6b4a', '#ffb84d', '#f5f2ea'].map((c) => (
            <motion.span
              key={c}
              animate={activeTool === 'eyedropper' && c === '#34d3ff' ? { scale: [1, 1.35, 1] } : { scale: 1 }}
              transition={{ duration: 0.6, repeat: activeTool === 'eyedropper' ? Infinity : 0 }}
        ...
        <AnimatePresence initial={false}>
          {layers.map((layer, i) => (
            <motion.div
              ...
              className={`absolute right-[-14px] flex items-center gap-2 rounded-lg border border-line ${layer.tone} px-3 py-2 backdrop-blur-md shadow-lg`}
            >
              <layer.icon className="h-3.5 w-3.5 text-ink-dim" strokeWidth={2} />
